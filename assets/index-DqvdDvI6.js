(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))l(i);new MutationObserver(i=>{for(const r of i)if(r.type==="childList")for(const o of r.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&l(o)}).observe(document,{childList:!0,subtree:!0});function t(i){const r={};return i.integrity&&(r.integrity=i.integrity),i.referrerPolicy&&(r.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?r.credentials="include":i.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function l(i){if(i.ep)return;i.ep=!0;const r=t(i);fetch(i.href,r)}})();const g={yougain:{title:"YouGain — Distributor Application",subtitle:"ATILIM LLC is seeking distribution partners for YouGain safety gloves.",who:"We are looking for country distributors in Europe and regional distributors in Africa.",geoNote:"First we separate as <strong>Europe</strong> and <strong>Africa</strong> and call it <strong>(EMEA)</strong>. We continue as <strong>EMEA</strong> below.",targetsHtml:`
    ...
    `,roleLabel:"Become a Partner (Solar)"}};function v(){const a=(new URLSearchParams(window.location.search).get("division")||"yougain").toLowerCase();return a==="industrial"||a==="promotional"||a==="solar"||a==="yougain"?a:"yougain"}function m(e){return e.replace(/[&<>"']/g,a=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[a])}function h(e){const a=v(),t=g[a];e.innerHTML=`
        ...
        <div class="apply_grid">
          ...
          <div class="apply_card">
            ...
            <form class="apply_form" name="distributor-application" method="POST" data-netlify="true" netlify-honeypot="bot-field">
              ...
              <input type="hidden" name="division" value="${m(a)}" />
  ...
  `;const l=e.querySelector("form"),i=e.querySelector(".apply_status");l.addEventListener("submit",async r=>{r.preventDefault(),i.textContent="Submitting…";try{const o=new FormData(l),u=new URLSearchParams(o).toString();if(!(await fetch("/",{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:u})).ok)throw new Error("Network error");i.textContent="Thank you! Your application has been received.",l.reset()}catch{i.textContent="Sorry — submission failed. Please try again or email info@atilim.ge."}})}const s="/atilim-site/",c=document.querySelector("#app");if(!c)throw new Error("#app bulunamadı");location.pathname==="/apply"?h(c):c.innerHTML=`
  <header class="topbar">
    <div class="container topbar_inner">
      <a class="brand" href="#about" aria-label="ATILIM LLC">
        <img class="brand_logo" src="${s}atilim-logo.svg" alt="ATILIM LLC" />
      </a>
  ...
  <section class="hero">
    <div class="container hero_center">
      <div class="hero_lockup" role="banner" aria-label="ATILIM LLC hero">
        <img class="hero_logo" src="${s}atilim-logo.svg" alt="ATILIM logo" />
        <div class="hero_text">
  ...
  <section id="partners" class="section">
    <div class="container">
      ...
      <div class="partners_grid" aria-label="Partners logos">
        <img class="partner_logo" src="${s}partners/yougain.png" alt="YouGain" loading="lazy" />
        <img class="partner_logo" src="${s}partners/sca.png" alt="SCA" loading="lazy" />
        <img class="partner_logo" src="${s}partners/dp.png" alt="DP Innovation" loading="lazy" />
        <img class="partner_logo" src="${s}partners/atilim-arch.png" alt="ATILIM Architecture" loading="lazy" />
        <img class="partner_logo" src="${s}partners/terra.png" alt="Terra Vitalis" loading="lazy" />
      </div>
    </div>
  </section>
  ...
  `;let n=null;function d(){try{n||(n=new(window.AudioContext||window.webkitAudioContext));const e=n,a=e.createOscillator(),t=e.createGain();a.type="square",a.frequency.setValueAtTime(520,e.currentTime),t.gain.setValueAtTime(1e-4,e.currentTime),t.gain.exponentialRampToValueAtTime(.12,e.currentTime+.005),t.gain.exponentialRampToValueAtTime(1e-4,e.currentTime+.06),a.connect(t),t.connect(e.destination),a.start(),a.stop(e.currentTime+.07)}catch{}}function p(e){e.classList.add("is-pressed"),window.setTimeout(()=>e.classList.remove("is-pressed"),120)}document.querySelectorAll(".pressable").forEach(e=>{e.addEventListener("pointerdown",()=>{d(),p(e)})});document.querySelectorAll(".pressable_link").forEach(e=>{e.addEventListener("pointerdown",a=>{d();const t=a.currentTarget.closest(".card");t&&p(t)})});
